        ...
        <div class="max-w-2xl mx-auto px-4">
            <div class="bg-white rounded-[3rem] shadow-2xl overflow-hidden border-t-[12px] border-[#5850ec]">
                ...
                <div class="px-12 pt-12 pb-8 flex justify-between items-start">
                    <div>
                        <h2 class="text-2xl font-black text-[#1e293b] uppercase tracking-tighter">Edit Data Omset</h2>
                        <p class="text-xs font-bold text-[#94a3b8] mt-1">Perbarui data omset marketing Starindo</p>
                    </div>
                ...
                <form action="{{ route('omset.update', $omset->id) }}" method="POST" class="px-12 pb-12 space-y-8">
                    @csrf
                    @method('PUT')
                        ...
                        <div class="relative">
                            <select name="marketing_id" required
                                ...
                                @foreach ($marketings as $m)
                                    <option value="{{ $m->id }}" {{ $omset->marketing_id == $m->id ? 'selected' : '' }}>
                                        {{ $m->nama }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div class="space-y-3">
                            ...
                            <input type="date" name="periode_dari" value="{{ $omset->periode_dari }}" required
                        ...
                        <div class="space-y-3">
                            ...
                            <input type="date" name="periode_sampai" value="{{ $omset->periode_sampai }}" required
                                class="w-full bg-[#f1f5f9] border-none rounded-[1.5rem] p-5 font-bold text-slate-600 focus:ring-2 focus:ring-[#5850ec] shadow-inner uppercase">
                        </div>
                    </div>

                    <div class="space-y-3" x-data="{
                        displayValue: new Intl.NumberFormat('id-ID').format({{ $omset->nominal }}),
                        rawValue: '{{ (int) $omset->nominal }}',
                        formatRupiah(val) {
                        ...
                        <div class="bg-[#f1f5f9] rounded-[1.8rem] p-5 flex items-center shadow-inner group-within:ring-2 group-within:ring-[#5850ec] transition-all">
                            ...
                            <input type="text" x-model="displayValue" @input="updateValue($event)"
                                class="w-full bg-transparent border-none p-0 font-black text-2xl text-[#334155] focus:ring-0">
