            ...
            <div class="bg-white rounded-[3rem] shadow-2xl overflow-hidden border-t-8 border-blue-600">
                ...
                <div class="p-10 border-b border-slate-100 bg-slate-50/50">
                    <div class="flex justify-between items-center">
                        <div>
                            <h2 class="text-2xl font-black text-slate-800 uppercase tracking-tighter">Tambah Data Pengajuan</h2>
                ...
                <form action="{{ route('pengajuan.store') }}" method="POST" class="p-10 space-y-6">
                        ...
                        <div class="space-y-2">
                            ...
                            <select name="marketing_id" class="w-full bg-slate-50 border-none rounded-2xl p-4 font-bold text-sm focus:ring-2 focus:ring-blue-600 shadow-inner" required>
                                ...
                                @foreach($marketings as $m)
                                    <option value="{{ $m->id }}">{{ $m->nama }} (Lvl {{ $m->level }})</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="space-y-2">
                            <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Tanggal Transaksi</label>
                            <input type="date" name="tanggal" class="w-full bg-slate-50 border-none rounded-2xl p-4 font-bold text-sm focus:ring-2 focus:ring-blue-600 shadow-inner" required>
                        </div>
                    </div>
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                            <input type="text" name="customer_nama" placeholder="Nama Customer" class="w-full bg-slate-50 border-none rounded-2xl p-4 font-bold text-sm focus:ring-2 focus:ring-blue-600 shadow-inner" required>
                            <input type="text" name="customer_cp" placeholder="Kontak" class="w-full bg-slate-50 border-none rounded-2xl p-4 font-bold text-sm focus:ring-2 focus:ring-blue-600 shadow-inner">
                        </div>
                    </div>
                    ...
                    <div class="space-y-2">
                        ...
                        <select name="jenis_pengajuan" class="w-full bg-slate-50 border-none rounded-2xl p-4 font-bold text-sm focus:ring-2 focus:ring-blue-600 shadow-inner">
                            <option value="Komisi Penjualan">Komisi Penjualan</option>
                    ...
                    <div class="space-y-2">
                        ...
                        <textarea name="alamat" rows="3" class="w-full bg-slate-50 border-none rounded-2xl p-4 font-bold text-sm focus:ring-2 focus:ring-blue-600 shadow-inner" placeholder="Tulis alamat lengkap customer di sini..."></textarea>
                    </div>

                    <button type="submit" class="w-full py-6 bg-blue-600 text-white rounded-3xl font-black uppercase tracking-[0.2em] shadow-xl shadow-blue-200 hover:bg-blue-700 hover:scale-[1.02] transition-all">
                        Submit Pengajuan
                    </button>
                </form>
            </div>
