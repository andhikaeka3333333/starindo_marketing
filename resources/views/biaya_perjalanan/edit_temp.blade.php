<x-app-layout>
    <div class="py-8 bg-slate-50 min-h-screen" x-data="{
        ...
        displayNominal: '{{ number_format($data->nominal ?? 0, 0, ',', '.') }}',
        rawNominal: '{{ number_format($data->nominal ?? 0, 0, '', '') }}',
        ...
        <div class="max-w-3xl mx-auto px-4">
            <div class="bg-white rounded-[2.5rem] shadow-xl overflow-hidden border-t-8 border-blue-600">
                ...
                <div class="px-8 py-6 border-b border-slate-100 bg-slate-50/50 flex justify-between items-center">
                    <div>
                        <h2 class="text-xl font-black text-slate-800 uppercase tracking-tighter">Edit Draf Perjalanan
                        </h2>
                    ...
                    <a href="{{ route('biaya-perjalanan.create') }}"
                        class="text-slate-400 hover:text-slate-800 font-bold text-[10px] uppercase transition">Batal /
                        Kembali</a>
                </div>

                <form action="{{ route('biaya-perjalanan.updateTemp', [$type, $data->id]) }}" method="POST"
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div class="space-y-1">
                            <label class="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1">Marketing
                                Pemohon</label>
                        ...
                        <div class="space-y-1">
                            <label class="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1">Tgl &
                                Waktu Transaksi</label>
                            <input :type="type === 'tol' ? 'datetime-local' : 'date'" name="tanggal"
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div class="space-y-1">
                            <label class="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1">Nama
                                Customer</label>
                            <input type="text" name="customer_nama" value="{{ $data->customer_nama }}"
                                class="w-full bg-slate-50 border-none rounded-xl p-3 text-xs font-bold shadow-inner focus:ring-2 focus:ring-blue-600"
                                required>
                        </div>
                        <div class="space-y-1">
                            <label class="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1">Kontak /
                                CP</label>
                            <input type="text" name="customer_cp" value="{{ $data->customer_cp }}"
                                class="w-full bg-slate-50 border-none rounded-xl p-3 text-xs font-bold shadow-inner focus:ring-2 focus:ring-blue-600">
                        </div>
                    </div>

                    <div class="space-y-1">
                        <label class="text-[9px] font-black text-blue-600 uppercase tracking-widest ml-1">Kategori
                            Biaya</label>
                    ...
                    <template x-if="type === 'akomodasi'">
                        <div class="grid grid-cols-2 gap-4 p-5 bg-blue-50/50 rounded-2xl border border-blue-100">
                            ...
                            <div class="space-y-1">
                                <label class="text-[9px] font-black text-blue-600 uppercase ml-1"
                    ...
                    <template x-if="type === 'tol'">
                        <div class="space-y-4">
                            <div
                                class="p-4 bg-amber-50 rounded-2xl border-2 border-dashed border-amber-200 flex justify-between items-center">
                                <span class="text-[9px] font-black text-amber-600 uppercase tracking-widest">Kartu
                                    Tol:</span>
                                <span class="text-sm font-black text-amber-800"
                                    x-text="selectedMarketing.no_kartu_tol || 'BELUM DI-SET'"></span>
                            </div>
                            <div class="space-y-1 p-5 bg-amber-50/50 rounded-2xl border border-amber-100">
                                <label class="text-[9px] font-black text-amber-600 uppercase tracking-widest ml-1">Nama
                                    Gerbang Tol</label>
                                <input type="text" name="nama_gerbang" x-model="namaGerbang"
                                    :readonly="kategori === 'Top-Up Tol'"
                    ...
                    <template x-if="type === 'bensin'">
                        <div class="space-y-1 p-5 bg-emerald-50/50 rounded-2xl border border-emerald-100">
                            <label class="text-[9px] font-black text-emerald-600 uppercase tracking-widest ml-1">Posisi
                                KM Kendaraan</label>
                            <input type="number" name="km" x-model="km"
                                class="w-full bg-white border-none rounded-lg p-3 font-black text-emerald-700 shadow-sm focus:ring-2 focus:ring-emerald-500">
                        </div>
                    </template>
                    ...
                    <template x-if="type !== 'akomodasi'">
                        <div class="space-y-4">
                            <div class="space-y-1">
                                <label class="text-[9px] font-black text-blue-600 uppercase tracking-widest ml-1"
                                    x-text="type === 'tol' ? 'Update Biaya Tol' : 'Update Nominal Rupiah'"></label>
                                <div class="relative">
                                    <span
                            ...
                            <div class="space-y-1">
                                <label
                                    class="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1">Keterangan
                                    / Catatan</label>
                                <textarea name="keterangan" rows="2"
                                    class="w-full bg-slate-50 border-none rounded-xl p-3 text-xs font-bold shadow-inner focus:ring-2 focus:ring-blue-600">{{ $data->keterangan }}</textarea>
                            </div>
                        </div>
                    </template>

                    <div
                        class="px-6 py-4 rounded-2xl bg-indigo-600 flex justify-between items-center shadow-lg transition-all">
                        <div class="flex flex-col">
                            <span class="text-[9px] font-black text-indigo-200 uppercase tracking-widest">Update
                                Perubahan</span>
                            <span
                                class="text-[9px] font-black text-white bg-indigo-500 px-2 py-0.5 rounded mt-0.5 shadow-sm inline-block"
                                x-text="'Kategori: ' + kategori"></span>
                        </div>
                        <div class="text-right">
                            <p class="text-[8px] font-bold text-indigo-300 uppercase mb-1 tracking-tighter">Total
                                Perhitungan</p>
