<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-extrabold text-2xl text-slate-800 tracking-tighter uppercase italic">Monitoring Biaya Perjalanan</h2>
    ...
    <div class="py-8 bg-slate-50 min-h-screen" x-data="{ tab: 'akom' }">
        <div class="max-w-[1400px] mx-auto px-4">
            ...
            @if(session('success'))
                <div class="mb-6 p-4 bg-white border-l-8 border-emerald-500 shadow-lg rounded-2xl text-slate-800 font-black italic uppercase text-xs tracking-widest">
                    {{ session('success') }}
                </div>
            @endif
                            ...
                            <tbody class="divide-y divide-slate-50">
                                @forelse($akomodasi as $a)
                                    <tr class="hover:bg-indigo-50/30 transition-colors">
                                        ...
                                        <td class="py-5 px-4 font-black uppercase italic">{{ $a->marketing->nama ?? 'N/A' }}</td>
                                        <td class="py-5 px-4 font-black text-indigo-600 uppercase">{{ $a->customer_nama }}</td>
                                        <td class="py-5 px-4 font-bold text-slate-400 italic">{{ $a->customer_cp ?? '-' }}</td>
                                        <td class="py-5 px-4 uppercase font-medium text-slate-500">{{ $a->wilayah }}</td>
                                        <td class="py-5 px-4 font-bold italic text-slate-700 uppercase">{{ $a->kategori }}</td>
                                        <td class="py-5 px-4 text-center font-bold text-slate-600">{{ $a->durasi }} Hari</td>
                                        <td class="py-5 px-4 text-right font-black text-sm text-slate-900 italic">Rp {{ number_format($a->nominal, 0, ',', '.') }}</td>
                                        <td class="py-5 px-4">
                                ...
                                @empty
                                    <tr><td colspan="9" class="py-20 text-center italic text-slate-300 font-bold uppercase tracking-widest">Data Akomodasi Kosong</td></tr>
                                @endforelse
                            </tbody>
                        </table>
                            ...
                            <tbody class="divide-y divide-slate-50">
                                @forelse($tol as $t)
                                    <tr class="hover:bg-amber-50/30 transition-colors">
                                        <td class="py-5 px-4 font-bold text-slate-400">{{ date('d/m/y', strtotime($t->tanggal)) }}</td>
                                        <td class="py-5 px-4 font-black uppercase italic">{{ $t->marketing->nama ?? 'N/A' }}</td>
                                        <td class="py-5 px-4 font-black text-amber-800 italic tracking-tighter">{{ $t->marketing->no_kartu_tol ?? '-' }}</td>
                                        <td class="py-5 px-4 font-black text-slate-800 uppercase">{{ $t->customer_nama }}</td>
                                        <td class="py-5 px-4 font-bold text-slate-400 italic">{{ $t->customer_cp ?? '-' }}</td>
                                        <td class="py-5 px-4 font-bold text-amber-600 uppercase italic">{{ $t->kategori }}</td>
                                        <td class="py-5 px-4 max-w-[150px] truncate uppercase font-medium text-slate-500">{{ $t->keterangan ?? '-' }}</td>
                                        <td class="py-5 px-4 text-right font-black text-sm text-slate-900 italic">Rp {{ number_format($t->nominal, 0, ',', '.') }}</td>
                                        <td class="py-5 px-4 text-right">
                                            <div class="flex justify-end gap-4">
                                                <a href="{{ route('biaya-perjalanan.edit', ['tol', $t->id]) }}" class="text-slate-300 hover:text-blue-600 transition-colors">
                                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="3"><path d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"></path></svg>
                                                </a>
                                                <form action="{{ route('biaya-perjalanan.destroy', ['tol', $t->id]) }}" method="POST" onsubmit="return confirm('Hapus data tol?')">
                                                    @csrf @method('DELETE')
                                                    <button type="submit" class="text-slate-300 hover:text-red-600 transition-colors">
                                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="3"><path d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg>
                                                    </button>
                                ...
                                @empty
                                    <tr><td colspan="9" class="py-20 text-center italic text-slate-300 font-bold uppercase tracking-widest">Data Tol Kosong</td></tr>
                                @endforelse
                            </tbody>
                        </table>
                            ...
                            <tbody class="divide-y divide-slate-50">
                                @forelse($bensin as $b)
                                    <tr class="hover:bg-emerald-50/30 transition-colors">
                                        ...
                                        <td class="py-5 px-4 font-black uppercase italic">{{ $b->marketing->nama ?? 'N/A' }}</td>
                                        <td class="py-5 px-4 font-black text-slate-800 uppercase">{{ $b->customer_nama }}</td>
                                        <td class="py-5 px-4 font-bold text-slate-400 italic">{{ $b->customer_cp ?? '-' }}</td>
                                        <td class="py-5 px-4 text-center font-black text-emerald-700 italic">{{ number_format($b->km, 0, ',', '.') }} KM</td>
                                        <td class="py-5 px-4 max-w-[200px] truncate uppercase font-medium text-slate-500">{{ $b->keterangan ?? '-' }}</td>
                                        <td class="py-5 px-4 text-right font-black text-sm text-slate-900 italic">Rp {{ number_format($b->nominal, 0, ',', '.') }}</td>
                                        <td class="py-5 px-4 text-right">
                                            <div class="flex justify-end gap-4">
                                ...
                                @empty
                                    <tr><td colspan="8" class="py-20 text-center italic text-slate-300 font-bold uppercase tracking-widest">Data Bensin Kosong</td></tr>
                                @endforelse
                            </tbody>
                        </table>
                            ...
                            <tbody class="divide-y divide-slate-50">
                                @forelse($operasional as $o)
                                    <tr class="hover:bg-blue-50/30 transition-colors">
                                        ...
                                        <td class="py-5 px-4 font-black uppercase italic">{{ $o->marketing->nama ?? 'N/A' }}</td>
                                        <td class="py-5 px-4 font-black text-slate-800 uppercase">{{ $o->customer_nama }}</td>
                                        <td class="py-5 px-4 font-bold text-slate-400 italic">{{ $o->customer_cp ?? '-' }}</td>
                                        ...
                                        <td class="py-5 px-4 text-right font-black text-sm text-slate-900 italic">Rp {{ number_format($o->nominal, 0, ',', '.') }}</td>
                                        <td class="py-5 px-4 text-right">
                                            <div class="flex justify-end gap-4">
                                ...
                                @empty
                                    <tr><td colspan="8" class="py-20 text-center italic text-slate-300 font-bold uppercase tracking-widest">Data Operasional Kosong</td></tr>
                                @endforelse
                            </tbody>
                        </table>
